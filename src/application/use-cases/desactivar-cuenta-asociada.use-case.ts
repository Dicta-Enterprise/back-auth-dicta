...
@Injectable()
export class DesactivarCuentaAsociadaUseCase {
  ...
  async execute(id: number, idpadre: number) {
    try {
      ...
      const actualizada = await this.service.cambiarEstado(id, 'INACTIVA');
      return Result.ok(actualizada);
    } catch {
      return Result.fail(new Error('Ocurrió un error desconocido al desactivar la cuenta asociada.'));
    }
  }
}
